...
class TestMatlabConnect:
    def test_reports_not_installed_when_missing(self, monkeypatch):
        monkeypatch.setattr("ion.drivers.matlab.driver.shutil.which", lambda _: None)
        driver = MatlabDriver()
        info = driver.connect()
...
class TestMatlabRunFile:
    def test_uses_matlab_batch(self, monkeypatch):
        monkeypatch.setattr(
            "ion.drivers.matlab.driver.shutil.which",
        ...
        monkeypatch.setattr("ion.runner.subprocess.run", fake_run)
