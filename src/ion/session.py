"""Session client — HTTP client that talks to ion-server.

Always HTTP, whether local or remote:
  ion connect --solver pyfluent                    # auto-starts ion-server locally
  ion connect --solver pyfluent --host 100.90.x.x  # talks to remote ion-server
...
class SessionClient:
    """HTTP client for ion-server. Works with local or remote servers."""
    ...
    def _auto_start_server(self) -> bool:
        """Start ion-server locally as a background process."""
        cmd = [sys.executable, "-c",
               "import uvicorn; from ion.server import app; "
    ...
    def _request(self, method: str, path: str, timeout: float = CMD_TIMEOUT_S, **kwargs) -> dict:
        try:
            ...
            return {"ok": False, "error": f"cannot reach ion-server at {self._base}"}
    ...
    def connect(self, solver: str, mode: str = "meshing",
        ...
        if self._is_local() and not self._server_reachable():
            if not self._auto_start_server():
                return {"ok": False, "error": "failed to auto-start ion-server locally"}
