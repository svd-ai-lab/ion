"""ion CLI — unified interface for LLM agents to control CAD/CAE simulation software."""
...
def _get_store() -> RunStore:
    root = os.environ.get("ION_DIR", str(Path.cwd() / ".ion"))
...
@click.option("--host", envvar="ION_HOST", default=None,
              help="Remote ion-server host (e.g. 100.90.110.79). Default: localhost (auto-start).")
@click.option("--port", envvar="ION_PORT", default=7600, type=int,
              help="ion-server port (default: 7600).")
@click.pass_context
...
def serve(serve_host, serve_port):
    """Start the ion HTTP server (like ollama serve)."""
    import uvicorn
    from ion.server import app

    click.echo(f"[ion] server starting on {serve_host}:{serve_port}")
    if serve_host == "0.0.0.0":
        click.echo("[ion] accessible on network (Tailscale)")
...
def check(ctx, solver):
    ...
    if ctx.obj["json"]:
        ...
        if info.status == "ok":
            click.echo(f"[ion] check: {info.message}")
        else:
            click.echo(f"[ion] check: {info.message}", err=True)
            sys.exit(1)
...
def lint(ctx, script):
    ...
    from ion.drivers import DRIVERS
    ...
    if ctx.obj["json"]:
        ...
        click.echo(f"[ion] lint: {'passed' if result.ok else 'failed'}")
...
def run(ctx, script, solver):
    ...
    if ctx.obj["json"]:
        ...
        click.echo(f"[ion] run:    {script} via {solver}")
        click.echo(f"[ion] status: {status} ({result.duration_s}s)")
        click.echo(f"[ion] log:    saved as #{run_id}")
        if result.exit_code != 0 and result.stderr:
            click.echo(f"[ion] stderr: {result.stderr}")
...
def connect(ctx, solver, mode, ui_mode, processors):
    ...
    if ctx.obj["json"]:
        ...
        if result.get("ok"):
            click.echo("[ion] connect: session ready")
            if result.get("data"):
                click.echo(json_mod.dumps(result["data"], indent=4, default=str))
        else:
            click.echo(f"[ion] connect: failed — {result.get('error', 'unknown')}", err=True)
            sys.exit(1)
...
def exec_cmd(ctx, code, code_file, label):
    ...
    if not code:
        click.echo("[ion] error: provide code as argument or via --file", err=True)
        sys.exit(1)
...
def ps(ctx):
    ...
    if ctx.obj["json"]:
        ...
        if result.get("connected"):
            ...
            click.echo("[ion] no active session")
...
def disconnect(ctx):
    ...
    if ctx.obj["json"]:
        ...
        if result.get("ok"):
            ...
            click.echo(f"[ion] disconnected (session_id={sid})")
        else:
            click.echo(f"[ion] error: {result.get('error')}", err=True)
            sys.exit(1)
...
def logs(ctx, target, field):
    """Browse run history. Optionally show a specific run: ion logs last"""
    store = _get_store()
    ...
    if target:
        try:
            ...
            click.echo(f"[ion] error: {e}", err=True)
            sys.exit(1)
        parsed = record.get("parsed_output", {})
        if field:
            if field not in parsed:
                click.echo(f"[ion] error: field '{field}' not found", err=True)
                sys.exit(1)
            if ctx.obj["json"]:
        ...
        if not runs:
            if ctx.obj["json"]:
                ...
                click.echo("[ion] no runs recorded")
            return
        if ctx.obj["json"]:
